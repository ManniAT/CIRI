...
#define SIMPLE_MQTT_CALLBACK_SIGNATURE std::function<void(char*, char*)>
...
private:
	...
	static void MQTTCallback(char* topic, byte* payload, unsigned int length) {
		...
		_InstancePTR->_CallBack(topic, _ReceiveBuffer);
	}
...
public:
	...
	bool PublishJSON(char* pTopic, char* pValue) {
	...
	bool Publish(char* pTopic, char* pValue) {
		int nTopicLen = strnlen(pTopic, TOPIC_BUFFSIZE);
	...
	bool ReConnect() {
		if (PSClient.connect(_ClientID)) {
			// Once connected, resubscribe...
			// ... and resubscribe
			PSClient.subscribe(_SubscribedTopic);
		}
